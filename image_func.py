...
def water_edges(scr,ch):
    ...
    if(ch=='M'):
        thresh = cv2.adaptiveThreshold(out,255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,121,1)
    else:
        thresh = cv2.adaptiveThreshold(out,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,121,1)    
    ...
    # scr=np.int64(scr)
    # scr[thresh==255]=scr[thresh==255]+20
    # scr[scr>255]=255
    # scr=np.uint8(scr) 
    # scr=cv2.GaussianBlur(scr,(7,7),0)
    ...
    zeros_n = water = np.zeros([scr.shape[0],scr.shape[1],3],dtype=np.uint8)
    # water[markers == -1] = [255,255,255]
    ...
    # cv2.drawContours(Draw_contour,contours, 0, (0,255,0), 1)
...
def canny_2_for_wavelet(scr,ch):
    ...
    if(ch=='G'):
        thresh = cv2.adaptiveThreshold(im2,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C ,cv2.THRESH_BINARY,75,1)
    elif(ch=='M'):
        thresh = cv2.adaptiveThreshold(im2,255,cv2.ADAPTIVE_THRESH_MEAN_C  ,cv2.THRESH_BINARY,75,1)
    ...
    e= cv2.Canny(im2,avg_n*0.4,avg_n*0.7)
